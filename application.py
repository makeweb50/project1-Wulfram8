...
def index():
    ...
    if request.method == "POST":
        ...
        return render_template("index.html", result = result, nickname = session["nickname"], login = session["login"])
...
def registr():
    ...
    if request.method == "POST":
        ...
        if login and password:
            if not nickname:
                db.execute("INSERT INTO users (login, password, nickname) VALUES (:login, :password, :nickname)",
...
def signin():
    ...
    if request.method == "POST":
        login = request.form.get("login")
        password = request.form.get("password")
        response = db.execute("SELECT * FROM users WHERE login = :login AND password = :password",
...
def book(isbn):
    ...
    res = requests.get("https://www.goodreads.com/book/review_counts.json", params={"key": "MdlurN92OnpSsJSFX19YJQ", "isbns": isbn}).json()
    gr = {'average': "", 'rcount': ""}
    gr['average'] = res['books'][0]['average_rating']
    gr['rcount'] = res['books'][0]['work_ratings_count']
